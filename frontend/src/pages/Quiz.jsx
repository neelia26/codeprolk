...
export default function QuizPage() {
  ...
  useEffect(() => {
    fetch("/api/quiz/today", {
      ...
      .catch(() => { setMessage("Unable to load the quiz. Please refresh to try again."); setLoading(false); });
  }, []);
  ...
  const submit = async () => {
    ...
    try {
      ...
      setResult(data.is_correct === true ? "correct" : data.is_correct === false ? "incorrect" : "submitted");
      setQuiz(null);
      setSelected(null);
  ...
  if (result) {
    ...
    return (
      <section className="challenge-surface">
        <div className={`quiz-page quiz-status-page quiz-feedback quiz-feedback-${result}`}>
          {correct && (
          ...
          <div role="status" aria-live="polite" className="quiz-feedback-copy">
            <span className="quiz-status-mark" aria-hidden="true">{correct ? "✓" : incorrect ? "×" : "✓"}</span>
            <h2>{correct ? "Correct answer!" : incorrect ? "Not quite this time" : "Answer submitted"}</h2>
            <p>{correct ? "Well done! Your answer has been recorded." : incorrect ? "That answer was incorrect. Your attempt has been recorded." : "Your attempt has been recorded."}</p>
          </div>
    ...
    <section className="challenge-surface"><div className="quiz-page">
      ...
      <ul>
        {quiz.options.map((opt, idx) => (
          <li key={idx}>
            <label>
              <input type="radio" name="opt" disabled={submitting} checked={selected === idx} onChange={() => setSelected(idx)} />{" "}
              {opt}
            </label>
          </li>
        ))}
      </ul>
      <button onClick={submit} disabled={selected === null || submitting}>{submitting ? "Submitting..." : "Submit"}</button>
