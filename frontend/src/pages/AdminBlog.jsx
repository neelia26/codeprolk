  ...
  async function publish(event) {
    ...
    try {
      ...
      const response = await fetch("/api/admin/blog/posts", {
        ...
        body: JSON.stringify({
          caption: caption.trim(), image_base64: encoded,
        }),
      });
      ...
      setError(e.message || "Could not confirm publication. Check the Blog page before retrying.");
    } finally {
    ...
    <section className="cp-blog-publisher">
      ...
      <form onSubmit={publish}>
        <fieldset disabled={busy}>
          ...
          <input id="blog-image" type="file" accept="image/png,image/jpeg,image/webp"
            ref={fileInput} onChange={choose} required aria-describedby="blog-image-help" />
          <small id="blog-image-help">PNG, JPEG or WebP · Up to 5 MB</small>
          <label htmlFor="blog-caption">Caption</label>
          <textarea id="blog-caption" value={caption} rows={9} maxLength={12000}
            required onChange={(e) => setCaption(e.target.value)} />
          <small>The first non-empty line is your heading. Put the rest of your caption underneath it.</small>
          {preview && <figure className="cp-blog-preview">
